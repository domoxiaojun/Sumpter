...
test('remote Linux setup downloads authenticated installer and preserves failure before writes', async (t) => {
  ...
  const server = createServer((req, res) => {
    ...
    if (extensionMismatch && req.url.endsWith('.ts')) body = Buffer.from(body.toString().replace('SUMPTER_ATTRIBUTION_BUNDLE_VERSION: 0.4.5', 'SUMPTER_ATTRIBUTION_BUNDLE_VERSION: 0.0.0'));
    res.end(body);
  });
