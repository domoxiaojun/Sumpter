...
- 取消代理与 Admin 请求体的 64 MiB / 4 MiB 上限以及入站 WebSocket 帧/消息上限；压缩请求体按完整解压结果转发，实际容量受可用内存约束。
...
- Provider 入口支持指定固定解析 IP；HTTP、模型目录探测和 WebSocket 连接均保持 API 地址中的 Host、TLS SNI 与证书校验域名。

### 修复

- macOS daemon 初始加载配置时校验 `resolveIP`，非法地址不再等到请求转发阶段才失败。
...
- 关闭 runtime SQLite worker 时截断 WAL，避免下一个只读打开改写主库或看到残留 `-wal`。

## [0.4.16] - 2026-09-16

### 修复

- 修复 Claude Code 请求含消息级 `system` / `developer` 指令时被协议转换桥错误拒绝的问题。
- Chat、Responses 与 Gemini 转换现在保留消息级指令；跨入口故障切换时不丢失指令内容，并继续明确拒绝无法表达的内容块。

## [0.4.15] - 2026-09-15

### 修复

- Linux 运行事件列表在请求行内显示源 IP，覆盖进行中和历史事件，并重建随包分发的 Web 资源。
- 修复发布输入的版本同步遗漏和归因安装测试的硬编码版本，补齐 macOS 内置归因资源。
- v0.4.12 至 v0.4.14 未完成正式发布；本版包含其 Linux 展示修复。
...
- v0.4.10 未完成发布；本版补齐部署镜像版本示例，包含其全部修复。
...
- 容器首次启动时，`init` 会在日志里直接打印初始 Admin 密码与凭据路径，方便首次登录（只打印一次，改密后即失效）；不希望明文进容器日志时，先按 DOCKER.md 的「自备 `config.json` 与初始密码」自己生成凭据即可。原生 Linux 安装器与 macOS App 仍只显示密码文件路径、不回显密码值。
- Compose 部署模板精简：`stop_signal` 与 `restart` 不再重复声明，`healthcheck` 也不再重复声明，存活探针改由镜像的 `HEALTHCHECK` 提供（`docker compose up --wait` 仍按容器健康状态等待）；部署文档重整阅读顺序并压缩篇幅。
- 部署文档新增「自备 `config.json` 与初始密码」：已有配置或想自己控制初始密码时，先把两个文件放进 `./config`，`init` 会静默跳过；配置教程写明容器路线的 `./config` 挂载、将 `config.json` 拷入挂载目录，以及 `listener.host` 与 `admin-password` 两项容器专属要求；使用文档补充卸载章节。

## [0.4.6] - 2026-09-12

### 新增

- Linux WebUI 与 macOS 入口库支持分别设置 Anthropic、OpenAI（Chat / Responses 共用）和 Gemini 的 User-Agent；自动模式保留客户端 UA，在缺失时按配置补充，强覆盖模式替换客户端 UA。
- UA 设置应用于 HTTP、WebSocket 与入口模型探测；自动协议探测在原有总超时内按不同协议的 UA 尝试获取模型。

### 修复

- 双端对 UA 长度与控制字符统一校验，保存时清理两端空白；旧配置缺少 UA 字段仍可加载，修改 UA 后模型目录缓存及时失效。
- Linux 分流规则从所选入口的已配置或已获取模型中选择，切换入口清空旧模型，保存时拒绝失效选项。
- 修复 Linux 运行页进行中请求耗时显示，并调整 UA 编辑器布局与入口摘要。
- 补齐共享协议校验对 Gemini 的支持，同步 Linux 使用文档与部署契约。

### 变更

- 运行镜像（`Dockerfile` 与 `Dockerfile.runtime`）安装 `tzdata`，容器内 `TZ` 现在生效；v0.4.5 及更早的发布镜像没有 tzdata。程序自身的统计与时间戳仍按 Unix 时间（UTC）存储，日志时间戳也是 UTC，WebUI 按浏览器时区展示。
- 部署文档补充“宿主机侧 vs 容器内”端口说明：proxy（数据面）与 admin（控制面）是 daemon 的两个独立监听项，不是重复变量；代理在容器内的监听端口来自 `config.json`，不应在 WebUI 修改，否则端口映射失效。

### 兼容性

- 配置继续使用 schema v7，runtime 数据库格式不变；未设置 UA 规则时沿用原有转发行为。
- Compose 模板由本地契约检查及 Linux CI 验证；目标主机的首次启动、登录、实际代理流量和停机迁移仍需真机验收。
...
- macOS 界面预览测试使用独立临时目录并自动清理，修复因开发机绝对路径导致的 Release runner 测试失败。
- 包含 0.4.3 的模型组随机/轮询粘性调度和 Pi 归因脚本改进；0.4.3 标签保留，安装包发布由本版本完成。

## [0.4.3] - 2026-09-11

### 新增

- 模型组支持同优先级入口随机粘性 `randomSticky` 和顺序轮询粘性 `roundRobinSticky`；新会话分配后保持入口归属，五个并发新会话可依次使用五个同级调度组。
- 轮询首次取号与会话归属原子建立，同一会话并发首请求只占一个位置；重试不消耗轮询位置，失败后按原有规则切换并重新绑定。
- Linux WebUI 与 macOS 模型组编辑器同步提供策略选择，未配置时继续采用优先级顺序。

### 改进与修复

- 双端统计筛选收起高级选项，事件列表区分进行中与历史记录，缓存读取旁显示命中率；完善分页、窄窗口展示和请求链详情。
- 按项目或会话清除粘性归属；修复 macOS 重复事件 ID，以及入口模型范围残留选择重新授权已删除映射的问题。
- Pi 包管理、配置和鉴权命令由包装器原样透传，避免被当作聊天提示；Linux 归因安装器增加卸载操作，并在安装前核对安装器与扩展资源版本。

### 兼容性

- 配置继续使用 schema v7；本版不改变 runtime 数据库格式。
- 轮询游标仅存在于当前进程，重启或重新加载配置后归零；已有有效会话归属保留。共享 `stickyGroup` 的入口继续作为一个调度组，组内按配置顺序尝试。
