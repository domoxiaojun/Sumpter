...
首次生成密码时，`docker compose logs init` 会给出文件路径与宿主机查看命令（例如 `sudo cat ./config/admin-password`）。按项目惯例日志只给路径，**不回显密码值**：值一旦进日志，就会留在 Docker 日志文件、日志采集系统和别人粘贴的排错输出里。后续启动不再重复提示，因为不会重新生成。
