...
首次生成密码时，`docker compose logs init`（或前台 `docker compose up` 的输出）会**直接打印初始密码**并给出文件路径：容器里没有安装器那样的交互终端，让用户去宿主机 `sudo cat` 是本路线最大的首次使用摩擦点。明文只打印一次，重启不会重复；**登录后请立即在 WebUI「安全」页修改**，改密后该文件变成 Argon2 哈希 JSON，日志里的初始密码随即失效。原生 Linux/macOS 安装只显示密码文件路径、不回显密码值，这一区别是有意的：容器日志由 Docker 管理且可以随时导出，而安装器在终端里能交互。如果你不希望明文出现在容器日志里（例如日志会被采集、转发或长期保留），用下面的「精简部署：自备 `config.json`，不用 `init`」自行生成密码。
...
也不需要 `.env`。代价是下面第 2、3 步必须你自己做完，做漏了不会自愈。这条路线顺带把密码留在
日志之外：没有 `init`，就没有任何组件会把明文写进容器日志。
...
| 初始密码在哪 | `init` 日志（首次一次）+ 文件 | 只有文件，你自己生成时就知道 |
