...
/**
 * Return exactly one state.  `hasMapping` means an enabled endpoint has at
 * least one explicit client mapping; an empty mapping never becomes a hidden
 * catch-all route.
 */
export function getOnboardingState({ status, config, runtime } = {}) {
  ...
  const hasMapping = endpoints.some((endpoint) => (
    endpoint
    && endpoint.enabled !== false
    && Array.isArray(endpoint.mappings)
    && endpoint.mappings.length > 0
  ));
