...
export function UnifiedAttributionPanel() {
  ...
  return <section className="glass-panel panel-padded-stack attribution-panel">
    ...
    <div className="attribution-code" aria-label="下载安装命令">
    ...
    {commands.map(([label, command]) => <div key={label} className="attribution-command">
    ...
    <p>脚本自动获取配套安装器与 pi 私有扩展。安装前备份终端配置；还原仅恢复所选客户端的归因块，保留其他配置。安装或还原后请新开终端并重新启动客户端。状态检查不等同于已观察到请求归因。</p>
    <p>pi 包装器自动加载私有配套扩展并启用归因，无需另行安装，不写入 pi 全局扩展目录；安装器不修改 provider 配置。/reload 不会加载 shell 配置，请从新终端启动 pi。</p>
