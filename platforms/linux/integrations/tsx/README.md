...
`kekulv-inflight.tsx` 使用 TypeScript 读取 Linux Admin API，只输出：
...
只需要 Node.js 18+ 和 `tsx` 运行器。直接编辑 `kekulv-inflight.tsx` 顶部的 `CONFIG`，然后运行：
...
```bash
KEKULV_ADMIN_URL='https://admin.example.com' \
KEKULV_ADMIN_USERNAME='kkl' \
KEKULV_ADMIN_PASSWORD='从安全凭据管理器注入' \
npx tsx platforms/linux/integrations/tsx/kekulv-inflight.tsx
```
