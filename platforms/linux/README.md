...
Linux 版以后台进程运行，通过 WebUI 管理入口、模型组、请求与统计。本文对应 **0.4.18 / schema v7**，支持 x86_64 / aarch64 的静态 musl 发布包。
...
需要固定版本时追加 `--version v0.4.18`。安装器也接受 `--admin-host`、`--admin-port`、`--admin-password-file`，将管理设置写入 systemd drop-in。升级时原有配置与密码会保留。
