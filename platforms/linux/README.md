...
Linux 版以 standalone daemon 提供多协议代理、入口库、多个模型组、分流规则、failover、统计，以及与桌面 UI 信息架构对齐的本机 Web 管理界面。版本与 schema 与仓库根一致（现为 0.4.9 / schema v7）。用户安装看下文「自动安装、升级与卸载」。
...
# 省略 --version 即下载 latest；钉死版本再加 --version v0.4.9
...
| `--version vX.Y.Z` | 改为该 tag 的 Release 资产，例如 `.../download/v0.4.9/` |
