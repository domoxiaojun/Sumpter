...
适用版本：**0.4.20 / schema v7**。本文随发布包提供，可在解压目录阅读；安装入口见 [README](README.md)。
...
安装入口：[Compose](DOCKER.md)、[Linux systemd](README.md)。
...
「Claude Code 路由」中的 WebSearch、WebFetch 和安全分类器分流识别独立子请求，可为其选择目标模型和入口。第一次配置可先全部关闭。字段细节见 [本文的配置步骤](#2-配好第一个上游)。
...
进一步排查见 [Linux 安装与维护](README.md#故障排查) 或 [Compose 排障](DOCKER.md#故障排查)。报告问题时附平台、版本、安装方式、复现步骤和脱敏错误，不附真实配置或完整诊断捕获。
