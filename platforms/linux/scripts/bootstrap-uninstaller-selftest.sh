...
TEST_ROOT="$(mktemp -d "$TEST_BASE/kekulv-bootstrap-uninstall-test.XXXXXX")"
USER_HOME="$TEST_ROOT/home"
INSTALLED_SCRIPT="$USER_HOME/.local/share/kekulv/scripts/uninstall.sh"
...
cleanup() {
    ...
    trap - EXIT
    case "$TEST_ROOT" in
        "$TEST_BASE"/kekulv-bootstrap-uninstall-test.*)
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >"${KEKULV_BOOTSTRAP_UNINSTALL_MARKER:?}"
EOF
chmod 0755 "$INSTALLED_SCRIPT"

HOME="$USER_HOME" \
    KEKULV_BOOTSTRAP_UNINSTALL_MARKER="$MARKER" \
    bash "$BOOTSTRAP"
[[ "$(cat "$MARKER")" == "" ]]
