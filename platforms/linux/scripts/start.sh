...
#
# 环境变量:
#   KEKULVD_BIN          可执行文件(默认:部署目录/kekulvd)
#   KEKULVD_PID_FILE     pidfile(默认:部署目录/kekulvd.pid)
#   KEKULVD_LOG_DIR      日志目录(默认:部署目录/logs)
#   KEKULVD_CONFIG_DIR   非空时追加 --config-dir
#   KEKULVD_WEB_ROOT     Web 根目录(默认:部署目录/web；可用 CLI --no-web 禁用)
#   KEKULVD_START_TIMEOUT  就绪等待秒数(默认 15)
#   KEKULV_ADMIN_HOST    Admin 绑定地址(默认 127.0.0.1；透传给 daemon)
#   KEKULV_ADMIN_PORT    Admin 端口(默认 57879；透传给 daemon)
#   KEKULV_ADMIN_PASSWORD_FILE 高级凭据路径覆盖；默认读取配置目录/admin-password
# 其余参数原样透传给 kekulvd（含 --admin-host / --admin-port / --admin-password-file）。
set -euo pipefail
umask 077

BASE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
BIN="${KEKULVD_BIN:-$BASE_DIR/kekulvd}"
PID_FILE="${KEKULVD_PID_FILE:-$BASE_DIR/kekulvd.pid}"
LOG_DIR="${KEKULVD_LOG_DIR:-$BASE_DIR/logs}"
LOG_FILE="$LOG_DIR/kekulvd.log"
WEB_ROOT="${KEKULVD_WEB_ROOT:-$BASE_DIR/web}"
START_TIMEOUT="${KEKULVD_START_TIMEOUT:-15}"
...
pid_is_kekulvd() {
    ...
    [[ "$(ps -p "$pid" -o comm= 2>/dev/null | awk '{print $1}')" == "kekulvd" ]]
}

terminate_spawned_pid() {
    local pid="$1"
    if ! pid_is_running "$pid" || ! pid_is_kekulvd "$pid"; then
        return
    fi
...
resolve_admin_probe() {
    ...
    if [[ -n "${KEKULV_ADMIN_HOST:-}" ]]; then
        host="$KEKULV_ADMIN_HOST"
    fi
    if [[ -n "${KEKULV_ADMIN_PORT:-}" ]]; then
        port="$KEKULV_ADMIN_PORT"
    fi
...
is_positive_integer "$START_TIMEOUT" || die "KEKULVD_START_TIMEOUT 必须是正整数"

resolve_admin_probe "$@"

[[ -x "$BIN" ]] || die "找不到可执行文件 $BIN(可用 KEKULVD_BIN 覆盖)"
BIN_REAL="$(readlink -f "$BIN" 2>/dev/null || readlink -m "$BIN")"
[[ -n "$BIN_REAL" ]] || die "无法解析 kekulvd 规范路径:$BIN"

if [[ -f "$PID_FILE" ]]; then
    old_pid="$(head -n 1 "$PID_FILE" 2>/dev/null || true)"
    if pid_is_running "$old_pid"; then
        if pid_is_kekulvd "$old_pid"; then
            echo "kekulvd 已在运行(pid $old_pid),无需重复启动。"
            exit 0
        fi
        die "pidfile 指向另一进程或另一份 kekulvd(pid $old_pid);为避免误伤,请人工核对 $PID_FILE"
    fi
...
if [[ -s "$LOG_FILE" ]]; then
    mv "$LOG_FILE" "$LOG_DIR/kekulvd-$(date +%Y%m%d-%H%M%S).log"
fi

daemon_args=()
if [[ -n "${KEKULVD_CONFIG_DIR:-}" ]]; then
    daemon_args+=(--config-dir "$KEKULVD_CONFIG_DIR")
fi
...
if [[ "$ready" -ne 1 ]]; then
    ...
    rm -f "$PID_FILE"
    echo "错误:kekulvd 未在 ${START_TIMEOUT}s 内就绪(Admin $ADMIN_BIND_DESC)。最近日志:" >&2
    tail -n 30 "$LOG_FILE" >&2 || true
    exit 1
fi

echo "kekulvd 已后台启动(pid $pid)"
