...
正式包的检查入口有三处，走同一更新器：「关于」页的「检查更新…」、主窗口打开后应用菜单中的同名项，以及菜单栏图标右键菜单。未写入 `SUFeedURL` 的开发包会禁用这些入口，避免启动时弹出 Sparkle 配置错误。
...
```bash
export SPARKLE_DOWNLOAD_URL_PREFIX='https://github.com/domoxiaojun/sumpter/releases/download/v0.4.24/'
