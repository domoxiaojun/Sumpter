...
final class ClaudeHookEditingTests: XCTestCase {
    private let needle = "sumpter-notify.sh"
    ...
    func testUpsertPreservesExistingUserHooks() {
        ...
        let merged = ClaudeHookEditing.upsertCommand(
            "/bin/zsh /path/sumpter-notify.sh stop",
            matching: needle,
            into: existing
        )
    ...
    func testUpsertReplacesOldSumpterCommandWithoutDuplicating() {
        let existing: [[String: Any]] = [
            ...
            userRow("/old/path/sumpter-notify.sh stop")
        ]
        let merged = ClaudeHookEditing.upsertCommand(
            "/new/path/sumpter-notify.sh stop",
            matching: needle,
            into: existing
        )
        let sumpter = ClaudeHookEditing.commands(in: merged).filter { $0.contains(needle) }
        XCTAssertEqual(sumpter, ["/new/path/sumpter-notify.sh stop"], "旧Sumpter命令应被替换,不重复")
    ...
    func testRemovingKeepsUserHooksAndDropsSumpter() {
        let existing: [[String: Any]] = [
            userRow("/Users/me/my-own-hook.sh"),
            userRow("/path/sumpter-notify.sh stop")
        ]
    ...
    func testRemovingDropsEmptiedMatcherRows() {
        let existing: [[String: Any]] = [userRow("/path/sumpter-notify.sh stop")]
        let cleaned = ClaudeHookEditing.removingCommands(matching: needle, from: existing)
    ...
    func testUpsertIntoNilOrEmptyStartsFresh() {
        let merged = ClaudeHookEditing.upsertCommand("/path/sumpter-notify.sh notification", matching: needle, into: nil)
        XCTAssertEqual(ClaudeHookEditing.commands(in: merged), ["/path/sumpter-notify.sh notification"])
    }
}
