...
final class AdminClientNotificationTests: XCTestCase {
    ...
    func testGrokClientKindIsDecodedFromSSEWire() throws {
        let event = AdminWire.Event.parse(
            name: "notify",
            data: #"{"clientKind":"grok_build","title":"Grok Build · 回合完成","message":"回合已完成","sound":null,"type":"notification","category":"turn_completed","priority":"normal","actionID":null,"sessionId":"s-1","cwd":"/tmp/project"}"#
        )
        guard case .notify(let clientKind, let title, _, _, _, _, _, _, _, _) = event else {
            return XCTFail("expected notify event")
        }
        XCTAssertEqual(clientKind, "grok_build")
        XCTAssertEqual(title, "Grok Build · 回合完成")
    }
