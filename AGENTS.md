...
Unit tests live beside modules under `src/`; integration tests and fixtures live under each crate's `tests/`. `config.example.json` is the safe schema-v6 example. Treat `docs/upstream/` as migration reference, not the current API contract. Never commit `target/` artifacts.
