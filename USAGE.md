...
给第一次安装并接入客户端的用户。当前版本 **0.3.8**，配置 **schema v7**。
...
Claude Code、Grok Build、Gemini CLI、pi 共用仓库脚本。在**启动客户端的主机**操作，不要装到
只跑 daemon 的 Linux。本机 macOS App 可在「设置 → 安全」安装。

先临时跑（不改 rc）。从仓库拉 `client-attribution.mjs`，Base URL 填 daemon 可达地址：
...
| 怎么用 | App 一键安装，或先 `run` 再 `install` | 先 `client-attribution.mjs run`，要持久再 `setup-client-attribution.sh install` |
...
6. 需要项目统计时，在**启动客户端的主机**处理，不要装到只跑 daemon 的 Linux。可先 `node client-attribution.mjs run …` 临时跑；要持久再 `setup-client-attribution.sh install`。本机 App 也可用安全页。
