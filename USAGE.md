...
适用版本：**0.4.25 / schema v7**。从第一条请求开始，逐步配置客户端、模型组、项目归因和日常维护。
...
安装入口：[Compose](platforms/linux/DOCKER.md)、[Linux systemd](platforms/linux/README.md)、[macOS App](platforms/macos/README.md)。
...
「Claude Code 路由」中的 WebSearch、WebFetch 和安全分类器分流识别独立子请求，可为其选择目标模型和入口。第一次配置可先全部关闭。字段细节见 [配置参考](docs/configuration.md)。
...
进一步排查见 [故障排查](docs/troubleshooting.md)。报告问题时附平台、版本、安装方式、复现步骤和脱敏错误，不附真实配置或完整诊断捕获。
