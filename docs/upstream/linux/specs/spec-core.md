# Linux Rust 规格：SumpterCore 纯逻辑

> 同步蓝本：`macos/specs/spec-core.md`；来源提交与 Linux 有意差异记录在
> `RUST_UPSTREAM.md`。
> Linux 权威差异：默认路径改为 XDG `~/.config/kekulv`；当前只接受显式 schema v6
> `config.json`（自动迁移 v3/v4/v5）；仅有旧 `keys.json` 时拒启且不迁移；不使用 `.control_token`，Admin 密码来自独立文件；macOS
> `.autostart` 标记，systemd 自启动由 Admin 的固定 unit 接口管理。冲突时以本段和
> `admin-api.md` 为准。

> 正文保留上游 Rust 重写时的 Swift 行为对照，用于同步校对与回溯；它不代表本轮已经在
> Linux 编译或执行测试，当前验证边界以 `README.md` 为准。
...
| `RetryPolicy` | decode 即 clamp(轮数/时长/粘性重试 ≥0、并发 ≥1) | normalized() |
| `Endpoint.protocol` | 显式四态：`auto`（默认）/`anthropic`/`openai`/`openai-responses`；`auto` 只代表入口能力 | `EndpointProtocolMode`；实际 TargetFormat 仍为三种 `ProviderProtocol` |
...
| `AppConfig.endpoints` | 扁平 Provider 入口序列；磁盘无 `pools` | 同 |
...
responses 同规则拼 `/responses`。均加 `Accept: text/event-stream`。SourceFormat 只由入站路径
确定，不读取 UA 或 body 形状；路径与结构不匹配直接 `invalid_request`。
入口 `protocol` 先解析为真实 TargetFormat；只有 SourceFormat 与 TargetFormat 不同时才进入
Translator，非 200 原样 relay/走可重试。
...
RoutePlanner；原生候选存在时不会混入 Translator 候选，无法安全表达的工具、reasoning、引用
或未知内容块必须拒绝，禁止静默丢失。

## 9. ConfigStore（Linux 权威覆盖）

- 默认目录：`$XDG_CONFIG_HOME/kekulv`，否则 `~/.config/kekulv`；`--config-dir` 可覆盖。
- `config.json` 必须显式 `schemaVersion: 6`；pretty JSON、同目录临时文件、fsync、
  原子 rename、0600。若目录仅有 `keys.json`，报错退出且不读、不迁移、不改写。
- 配置与 secret 由 daemon/Admin 管理；GET 永不回明文，PUT 使用 generation 与独立
  `secretUpdates`，详见 `admin-api.md`。
- `ConfigDir` 中保留旧 `stats.json` 兼容读写辅助函数仅供历史模型测试/回滚代码使用；当前
  daemon 运行统计链路不调用它们，旧文件保持只读归档。当前 `runtime.sqlite3` 由 proxy 层管理。
- `kekulvd.pid` 由 daemon 写入配置目录；start/stop 部署脚本另有防误杀 pidfile。
- Linux 不生成或读取 `.control_token`；Admin 默认监听 `127.0.0.1:57879`（CLI/env 可覆盖）。
  配置目录 `admin-password` 初始化内置登录；缺失时拒绝启动。静态登录壳公开加载，API/SSE
  使用 HttpOnly 会话 Cookie，写请求同时校验 CSRF。

## 10. ConfigWarnings 与 InputValidation

- ConfigWarnings 全部规则已逐条移植(warnings.rs,文案逐字一致);顺序说明:分组类
  提示按首次出现序(Swift 字典无序),测试断言「包含」。
- 表单即时提示由 WebUI 负责；Linux Admin 仍必须在写盘前独立校验 listener、CIDR、URL、唯一 ID、模型映射、分流目标与超时/重试范围。
