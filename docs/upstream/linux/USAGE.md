...
<!-- BEGIN KEKULV_CANONICAL_ONBOARDING -->
...
|---|---|---|
| 配置文件 | `~/Library/Application Support/Sumpter/config.json`（仅在 macOS 客户端使用） | 普通用户：`$XDG_CONFIG_HOME/kekulv/config.json`，否则 `~/.config/kekulv/config.json`；system 安装：`/var/lib/kekulv/config.json` |
...
| 配置字段参考 | 源码 monorepo 的 `macos/CONFIG.md`（Linux 发布包不包含 macOS 文档） | 本文第 5 节的 `listener`、入口与 mapping 说明 |
| 项目归因脚本 | 发布包 `scripts/cc-project-attribution.sh`（通常 `/opt/kekulv/scripts/`）；必须在跑 Claude Code 的机器执行 | 发布包 `scripts/cc-project-attribution.sh`（通常 `/opt/kekulv/scripts/`）；必须在跑 Claude Code 的机器执行 |

本文件同时用于 monorepo 的 `linux/` 目录和独立 Linux 发布根；发布包不携带 macOS 文档。
...
<!-- END KEKULV_CANONICAL_ONBOARDING -->
...
发布仓库 [`domoxiaojun/sumpter`](https://github.com/domoxiaojun/sumpter) 的根就是本文件所在目录。
源码 monorepo 里请改仓库根 `USAGE.md`，再同步到这里；独立发布包只维护本文件。
...
|---|---|
| 开箱、接客户端、排错 | 本文 |
| 每个配置字段的含义 | 本文第 5 节；macOS 字段完整说明仅在源码树的 `macos/CONFIG.md` |
| Linux 安装、systemd、Docker、Admin 反代 | [`README.md`](README.md) |
| macOS 首次打开被拦截 | 该说明不随 Linux 发布包提供；请在源码树查看 `macos/app/INSTALL.txt` |
...
- **推荐抄** `linux/config.example.json`：域名是 `.invalid`，入口全部 `enabled: false`，secret 是合成的 `sk-test-…`。
- `macos/config.example.json` 更像本机草稿，可能带习惯用的上游域名、且入口是启用的。复制后务必换成你的地址和 key，不要原样拿去打真实流量。
...
不想用安装器时，把 App 拖到「应用程序」，再在 Finder 里右键 → 打开。不要用 `spctl --master-disable`，也不要对整个磁盘执行 `xattr -dr`。macOS 安装器的完整说明在源码树 `macos/app/INSTALL.txt`；Linux 发布包不包含该安装器。
...
更完整的 Linux 模板见 `config.example.json`；macOS 模板和字段逐项说明只在源码树 `macos/config.example.json` 与 `macos/CONFIG.md` 提供。
...
打包进 App 内的 `Sumpter.app/Contents/Resources/`，从源码构建则在 clone 仓库的 `linux/scripts/`
...
| 配置器位置 | App 内 `Sumpter.app/Contents/Resources/`（源码构建则是 clone 仓库的 `linux/scripts/`） | 发布包解包后 `/opt/kekulv/scripts/` |
