...
| 配置字段参考 | [`macos/CONFIG.md`](macos/CONFIG.md) | [`macos/CONFIG.md`](macos/CONFIG.md) |
| 项目归因脚本 | 源码树 `linux/scripts/cc-project-attribution.sh`；发布包按其 USAGE 路径 | 源码树 `linux/scripts/cc-project-attribution.sh`；发布包按其 USAGE 路径 |

本 monorepo 的 canonical 文档是 [`USAGE.md`](USAGE.md)；Linux 发布根会生成一份路径等价的 [`linux/USAGE.md`](linux/USAGE.md)。
...
发布仓库 [`domoxiaojun/sumpter`](https://github.com/domoxiaojun/sumpter) 的根就是 `linux/`，所以 GitHub
上这份指南在仓库根目录；本 monorepo 里同一份还放在 [`linux/USAGE.md`](linux/USAGE.md)，改的时候两份一起改。
...
|---|---|
| 开箱、接客户端、排错 | 本文 |
| 每个配置字段的含义 | [`macos/CONFIG.md`](macos/CONFIG.md)（两端同一份 schema） |
| Linux 安装、systemd、Docker、Admin 反代 | [`linux/README.md`](linux/README.md) |
| macOS 首次打开被拦截 | [`macos/app/INSTALL.txt`](macos/app/INSTALL.txt) |
| 改代码 / 发版 | [`CLAUDE.md`](CLAUDE.md)，不是使用文档 |
...
不想用安装器时，把 App 拖到「应用程序」，再在 Finder 里右键 → 打开。不要用 `spctl --master-disable`，也不要对整个磁盘执行 `xattr -dr`。细节见 [`macos/app/INSTALL.txt`](macos/app/INSTALL.txt)。
...
Docker、systemd、Admin HTTPS 反代、卸载见 [`linux/README.md`](linux/README.md)。
...
更完整的模板见 `macos/config.example.json` / `linux/config.example.json`。字段逐项说明见 [`macos/CONFIG.md`](macos/CONFIG.md)。
