# Sumpter Rust 后端(kekulvd)

Sumpter 3.0 的 Rust 后端 sidecar:本地 Anthropic 兼容代理引擎,由 SwiftUI 菜单栏壳
(`app/`)spawn 并通过 127.0.0.1 admin API 控制。重构计划见 `PLAN.md`,任务清单见 `todos.md`。

```bash
cargo build                  # debug 构建
cargo test                   # 全部测试
cargo test -p kekulv-core    # 单个 crate
...
`Sumpter.app/Contents/Resources/`，源码构建则是仓库的 `linux/scripts/`）。
...
- `kekulv-core` — 纯逻辑:配置模型(schema v6,与 `linux/` 同源同语义)、
  路由、粘性调度、访问控制、协议桥接纯函数。无网络无平台依赖。
- `kekulv-proxy` — tokio/axum/reqwest:入站服务、pinned-IP 出站、failover、
  SSE relay、统计事件、admin API。
- `kekulvd` — 可执行入口:组装、进程生命周期(stdin EOF 随父进程退出)。
