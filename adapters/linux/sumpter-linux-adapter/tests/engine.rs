...
fn engine_with_dir(
    ...
    Engine::new(config, Some(dir), fake)
}
...
async fn deferred_all_502_retries_next_round_and_recovers() {
    ...
    config.retry.max_deferred_rounds = 2;
    let engine = engine_with(config, fake.clone());
    // 上游真实 HTTP 502 属于统一 retryable 集；整轮失败后经退避进入下一轮。
    fake.push(
        "a.example.com",
        Outcome::Status {
            status: 502,
...
async fn tcp_client_disconnect_during_retry_backoff_records_499_and_stops() {
    ...
    let engine = engine_with(config, fake.clone());
    let (address, server) =
        sumpter_linux_adapter::server::serve(engine.clone(), SocketAddr::from(([127, 0, 0, 1], 0)))
            .await
            .unwrap();
...
async fn accepted_backfills_client_in_flight_endpoint_attribution() {
    ...
    let upstream = runtime
        ...
        .iter()
        .find(|e| e.kind == "upstream")
        .expect("accepted upstream in-flight event");
    assert_eq!(upstream.source_format, Some(ProviderProtocol::Anthropic));
    assert_eq!(upstream.target_format, Some(ProviderProtocol::Anthropic));
    assert_eq!(
        upstream.route_mode,
        Some(sumpter_core::routing::RouteMode::Native)
    );
    ...
    let upstream = runtime
        ...
        .iter()
        .find(|e| e.kind == "upstream")
        .expect("completed upstream event");
    assert_eq!(upstream.source_format, Some(ProviderProtocol::Anthropic));
    assert_eq!(upstream.target_format, Some(ProviderProtocol::Anthropic));
    assert_eq!(
        upstream.route_mode,
...
async fn native_endpoint_discards_translated_tool_candidate_before_forwarding() {
    ...
    assert_eq!(status, 200);
    // b 是原生 Anthropic 候选，因此规划阶段完全丢弃桥接候选 a。
...
#[tokio::test]
async fn linux_data_plane_has_no_control_or_notify_write_endpoints() {
    let fake = FakeTransport::new();
    let mut config = two_endpoint_config();
    config.listener.auth_token = "secret-inbound".into();
    let engine = engine_with(config, fake.clone());

    // Linux 管理写操作只走固定 loopback Admin listener，数据面无旁路。
    for path in ["/__reset-stats", "/__notify", "/__reload"] {
        let (status, _) = call(&engine, loopback(), path, vec![], Bytes::new()).await;
        assert_eq!(status, 404, "{path} 应在 Linux 数据面不存在");
    }
        ...
        runtime
            ...
            .all(|event| {
                event.phase == Some(RuntimeEventPhase::Completed)
                    ...
                    && event.source_format == Some(ProviderProtocol::Anthropic)
                    && event.target_format.is_none()
                    && event.route_mode.is_none()
            }),
        "Anthropic 入站拒绝事件必须结构化标记拒绝原因、保留原详情且不伪造未选择的 target/route"
    );
...
async fn rejected_requests_preserve_codex_metadata() {
    ...
    // 请求体故意无效：鉴权先拒绝时仍须把 header-only 元数据写入 client 事件。
    let (status, _) = call(
        &engine,
        loopback(),
        "/v1/responses",
    ...
    assert_eq!(event.source_format, Some(ProviderProtocol::OpenAIResponses));
...
/// WebSearch 指纹:必带 web_search 工具 + 强制 tool_choice(openai 系入口会被守卫跳过)。
...
#[tokio::test]
async fn webfetch_rule_bridges_on_openai() {
...
async fn claude_session_assignment_survives_request_error_context_change_and_restart() {
    ...
    let engine = Engine::new(config.clone(), Some(dir.clone()), fake.clone());
    ...
    let restarted = Engine::new(config, Some(dir.clone()), fake.clone());
    fake.push("a.example.com", sse_ok(&["data: {}\n\n"]));
    let before = fake.requests().len();
    assert_eq!(
        call(
...
async fn explicit_402_moves_session_once_then_sticks_to_replacement() {
    ...
    config.endpoints[0].sticky_group = Some("ga".into());
    config.endpoints[1].sticky_group = Some("gb".into());
    // 0 表示首次失败后切换；验证 CAS 本身:兜底成功立即迁移并粘住替代者。
    config.retry.session_sticky_retries = 0;
    let engine = engine_with(config.normalized(), fake.clone());
...
async fn sticky_group_retries_within_request_then_rebinds_immediately() {
    ...
    let engine = engine_with(config.normalized(), fake.clone());
    let session_id = stable_session("same-request-retries");
    let header = session_header(&session_id);

    // 同一个请求内:ga 首次尝试 + 2 次重试全部失败,之后才访问 gb。
    for _ in 0..3 {
        fake.push(
            "a.example.com",
            Outcome::Status {
...
async fn responses_inbound_bridges_tools_and_passes_client_ua() {
    ...
    for kind in ["client", "upstream"] {
        ...
        assert_eq!(metadata.subagent_kind.as_deref(), Some("thread_spawn"));
        assert_eq!(event.source_format, Some(ProviderProtocol::OpenAIResponses));
        assert_eq!(event.target_format, Some(ProviderProtocol::Anthropic));
        assert_eq!(
            event.route_mode,
            Some(sumpter_core::routing::RouteMode::Translated)
        );
...
// ---------------------------------------------------------------------------
// 与 macOS 侧对称的共享行为用例(2026-08-22 补齐)。
// 这些行为不含平台差异,两侧必须同时钉死;缺一侧等于该侧无保护。
// ---------------------------------------------------------------------------
...
async fn legacy_stats_is_ignored_and_preserved_across_requests_and_reset() {
    // admin-api.md 承诺旧 stats.json「不读取、不导入、不写入、不删除」。
    let fake = FakeTransport::new();
    let mut config = two_endpoint_config();
    config.endpoints.truncate(1);
    ...
    assert_eq!(
        engine.runtime_snapshot(),
        sumpter_core::events::RuntimeSnapshot::default()
    );
